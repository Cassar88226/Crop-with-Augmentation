...
class Pipeline(object):
    ...
    def _execute(self, augmentor_image, count, save_to_disk=True, multi_threaded=True):
        """
        ...
        if save_to_disk:
            file_name = os.path.basename(augmentor_image.image_path)
    ...
    def sample(self, count, multi_threaded=True):
        """
        ...
        if multi_threaded:
            ...
            with tqdm(total=len(augmentor_images), desc="Executing Pipeline", unit=" Samples") as progress_bar:
                for augmentor_image in augmentor_images:
                    self._execute(augmentor_image, count)
